...
`micro` is a canonical execution IR for DNN kernels. It represents a scheduled tensor program in terms of:
...
`micro.mma` represents matrix/tensor engine work one level above vendor ISA. It should describe tile shape, input dtype, accumulator dtype, and optional resource binding.

`micro.vector` represents vector elementwise work such as add, mul, exp, silu, reciprocal, or compare/select.

`micro.reduce` represents reductions such as sum, max, min, or product.
